...
export class CitizenProfileSyncService {
   ...
   */
  async generateProfileData(): Promise<CitizenProfileData> {
   ...
   */
  async syncToRegistry(profileData: CitizenProfileData): Promise<{
    success: boolean;
    ...
    try {
      ...
      const registryUpdate = await registryClient.submitExperimentalApplication({
   ...
   */
  async syncToEdenArt(profileData: CitizenProfileData): Promise<{
    success: boolean;
    response?: any;
    error?: string;
  }> {
...
export const citizenProfileSync = new CitizenProfileSyncService();