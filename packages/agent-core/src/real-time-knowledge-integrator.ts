...
import { miyomiRealTimeData, RealTimeContext } from './miyomi-realtime-data';
import { CitizenMarketData } from './citizen-market-data';
import { loadTrainingData, getEnhancedResponseContext } from './training-data-loader';
...
export interface AgentKnowledgeContext {
  ...
  lastUpdate: string;
  knowledgeFreshness: number; // 0-1 score
}
...
export class RealTimeKnowledgeIntegrator {
  private activeStreams: Map<string, KnowledgeStream> = new Map();
  private knowledgeCache: Map<string, AgentKnowledgeContext> = new Map();
  private updateIntervals: Map<string, NodeJS.Timeout> = new Map();
  private citizenMarketData: CitizenMarketData;
  
  // Real-time data sources
  private dataSources = {
    market: miyomiRealTimeData,
    citizen: new CitizenMarketData()
  };

  constructor() {
    this.citizenMarketData = new CitizenMarketData();
  ...
  private initializeDefaultStreams() {
    ...
    agentStreamConfigs.forEach(config => {
      config.streams.forEach(stream => {
        this.createKnowledgeStream(
          config.agentId,
          stream.type as 'market' | 'social' | 'creative' | 'governance',
          stream.frequency
        );
      });
    });
   ...
   */
  private startStream(streamKey: string): void {
   ...
   */
  private async fetchStreamUpdate(streamKey: string): Promise<void> {
    const stream = this.activeStreams.get(streamKey);
    if (!stream) return;

    let updates: LiveKnowledgeUpdate[] = [];
   ...
   */
  private async fetchMarketUpdates(agentId: string): Promise<LiveKnowledgeUpdate[]> {
    const updates: LiveKnowledgeUpdate[] = [];
   ...
   */
  private async fetchSocialUpdates(agentId: string): Promise<LiveKnowledgeUpdate[]> {
    const updates: LiveKnowledgeUpdate[] = [];
   ...
   */
  private async fetchCreativeUpdates(agentId: string): Promise<LiveKnowledgeUpdate[]> {
    const updates: LiveKnowledgeUpdate[] = [];
   ...
   */
  private async fetchGovernanceUpdates(agentId: string): Promise<LiveKnowledgeUpdate[]> {
    const updates: LiveKnowledgeUpdate[] = [];
   ...
   */
  private async updateAgentKnowledge(agentId: string, updates: LiveKnowledgeUpdate[]): Promise<void> {
    try {
      // Get or create agent context
      let context = this.knowledgeCache.get(agentId);
      
      if (!context) {
        const baseTrainingData = await loadTrainingData(agentId);
        context = {
          agent: agentId,
          baseTrainingData,
          realtimeUpdates: [],
   ...
   */
  private synthesizeKnowledge(context: AgentKnowledgeContext): any {
    const synthesis: any = {
      agent: context.agent,
    ...
    const updatesByType = context.realtimeUpdates.reduce((groups, update) => {
      if (!groups[update.updateType]) groups[update.updateType] = [];
      groups[update.updateType].push(update);
      return groups;
    }, {} as Record<string, LiveKnowledgeUpdate[]>);
   ...
   */
  async getRealTimeContext(agentId: string, query?: string): Promise<any> {
    ...
    if (query) {
      try {
        const trainingContext = await getEnhancedResponseContext(agentId, query);
        enhancedContext = {
          ...enhancedContext,
  ...
  // Helper methods
  private calculateOverallSentiment(updates: LiveKnowledgeUpdate[]): string {
  ...
  private calculateAverageVolatility(updates: LiveKnowledgeUpdate[]): number {
  ...
  private calculateFreshness(updates: LiveKnowledgeUpdate[]): number {
...
export const realTimeKnowledge = new RealTimeKnowledgeIntegrator();