...
exports.citizenSDK = exports.CitizenClaudeSDK = void 0;
const sdk_1 = __importDefault(require("@anthropic-ai/sdk"));
const sdk_2 = require("../registry/sdk");
const gateway_1 = require("../registry/gateway");
const flags_1 = require("../../config/flags");
const lore_manager_1 = require("../lore/lore-manager");
class CitizenClaudeSDK {
    constructor(apiKey) {
     ...
     */
    async generateProposal(topic, context, type) {
     ...
     */
    async analyzeConsensus(proposal, communityFeedback) {
        const prompt = `
...
Format as JSON:
{
  ...
  "potentialBlocks": [
     ...
     */
    async generateFellowshipStrategy(objective, timeframe) {
        const prompt = `
...
Format as JSON:
{
  ...
  "recommendations": ["recommendation1", "recommendation2"]
     ...
     */
    async syncWithRegistry(proposal) {
        try {
     ...
     */
    async processLoreUpdate(content, trainer) {
        try {
            const prompt = `
...
Maintain authenticity to Bright Moments values: provenance > speculation, IRL > Discord, fairness > favoritism`;
     ...
     */
    async processGovernanceUpdate(content, trainer) {
        try {
            const prompt = `
...
Maintain DAO governance principles and rough consensus philosophy`;
     ...
     */
    async processCommunityInsight(content, trainer) {
        try {
            const prompt = `
...
Remember: Ultra Full Set holders receive HIGHEST HONOR treatment`;
     ...
     */
    async processBrightMomentsUpdate(content, trainer) {
        try {
            const prompt = `
...
Create paths to consensus that respect minority viewpoints while enabling progress.`;
    ...
    buildProposalPrompt(topic, context, type) {
...
Provide as JSON:
{
  ...
  "metadata": {
    ...
    parseGovernanceProposal(response, type) {
        try {
     ...
     */
    async processLoreUpdate(content) {
        const prompt = `
...
Format as JSON:
{
  ...
  "communityImpact": "how_this_affects_community_engagement"
     ...
     */
    async processGovernanceUpdate(content) {
        const prompt = `
...
Format as JSON:
{
  ...
  "implementationSteps": ["step1", "step2", "step3"]
     ...
     */
    async processCommunityInsight(content) {
        const prompt = `
...
Format as JSON:
{
  ...
  "recognitionUpdates": ["update1", "update2"]
     ...
     */
    async processBrightMomentsUpdate(content) {
        const prompt = `
...
Format as JSON:
{
  ...
  "responseTemplates": ["template1", "template2"]
     ...
     */
    isSnapshotGovernanceEnabled() {
        const enabled = (0, flags_1.isFeatureEnabled)(flags_1.FLAGS.ENABLE_CITIZEN_SNAPSHOT_GOVERNANCE);
     ...
     */
    async createSnapshotProposal(topic, context, proposalType, spaceId = 'eden.eth') {
        ...
        try {
            ...
            const snapshotResult = await gateway_1.registryGateway.createSnapshotProposal(spaceId, snapshotProposal);
     ...
     */
    async getVotingPower(address, spaceId = 'eden.eth') {
        if (!this.isSnapshotGovernanceEnabled()) {
        ...
        try {
            const votingPower = await gateway_1.registryGateway.getSnapshotVotingPower(spaceId, address);
     ...
     */
    async coordinateVoting(proposalId, communityOutreach = true) {
        ...
        try {
            ...
            if (this.isSnapshotGovernanceEnabled()) {
                try {
                    proposal = await gateway_1.registryGateway.getSnapshotProposal(proposalId);
                }
                catch (error) {
...
Format as JSON:
{
  ...
  "timeline": "coordination_timeline"
     ...
     */
    async syncProposalResults(snapshotProposalId, registryWorkId) {
        if (!this.isSnapshotGovernanceEnabled()) {
        ...
        try {
            const syncResult = await gateway_1.registryGateway.syncSnapshotProposal(snapshotProposalId, registryWorkId);
     ...
     */
    async analyzeGovernancePerformance() {
        ...
        try {
            ...
            if (this.isSnapshotGovernanceEnabled()) {
                try {
                    const space = await gateway_1.registryGateway.getSnapshotSpace('brightmomentsdao-sepolia.eth');
    ...
    async chat(message, context) {
        ...
        try {
            ...
            const loreSystemPrompt = await lore_manager_1.loreManager.generateEnhancedSystemPrompt('citizen');
...
exports.CitizenClaudeSDK = CitizenClaudeSDK;
// Export singleton instance
exports.citizenSDK = new CitizenClaudeSDK(process.env.ANTHROPIC_API_KEY);
