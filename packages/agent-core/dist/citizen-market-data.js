...
exports.citizenMarketData = exports.CitizenMarketData = void 0;
const citizen_dune_integration_1 = require("./citizen-dune-integration");
class CitizenMarketData {
    constructor() {
     ...
     */
    async fetchRecentSales(slug, limit = 10) {
        if (!this.openSeaApiKey)
            return [];
        try {
            const headers = {
    ...
    async getAllCollectionStats() {
        ...
        const statsPromises = this.collections.map(collection => this.fetchOpenSeaStats(collection.slug));
    ...
    async getMarketSummary() {
        ...
        const [collectionStats, duneData] = await Promise.all([
            ...
            citizen_dune_integration_1.citizenDune.getBrightMomentsData()
        ]);
        ...
        const recentSalesPromises = this.collections.slice(0, 3).map(collection => this.fetchRecentSales(collection.slug, 5));
    ...
    calculateTrend(stats) {
        ...
        if (collectionsWithVolume.length === 0)
            return 'stable';
        const trendScore = collectionsWithVolume.reduce((score, stat) => {
            ...
            if (todayVolume > dailyAvg * 1.2)
                return score + 1;
            if (todayVolume < dailyAvg * 0.8)
                return score - 1;
            return score;
        }, 0);
        if (trendScore > collectionsWithVolume.length * 0.3)
            return 'bullish';
        if (trendScore < -collectionsWithVolume.length * 0.3)
            return 'bearish';
        return 'stable';
     ...
     */
    async getCityMarketData(city) {
        const collection = this.collections.find(c => c.city.toLowerCase().includes(city.toLowerCase()));
        if (!collection)
            return null;
     ...
     */
    async fetchSampleNFTs(slug, limit = 6) {
        if (!this.openSeaApiKey)
            return [];
        try {
            const headers = {
    ...
    async fetchWalletNFTs(walletAddress) {
        ...
        try {
            ...
            const brightMomentsNfts = processedNFTs.filter(nft => nft.collection_name.toLowerCase().includes('bright moments') ||
                ...
                nft.collection_name.toLowerCase().includes('crypto') &&
                    (nft.collection_name.toLowerCase().includes('venetians') ||
                        ...
                        nft.collection_name.toLowerCase().includes('londoners'))).length;
            // Count specifically CryptoCitizens
            const cryptocitizensCount = processedNFTs.filter(nft => nft.collection_name.toLowerCase().includes('cryptocitizens')).length;
            // Calculate total estimated value
            const totalValue = processedNFTs.reduce((sum, nft) => sum + (nft.floor_price || 0), 0);
     ...
     */
    async getBrightMomentsTreasuryHoldings() {
        ...
        try {
            ...
            const walletResults = await Promise.all(treasuryWallets.map(async (wallet) => {
     ...
     */
    async analyzeHolderValue(walletAddress) {
        ...
        let setStatus = 'single';
        const citizenCount = walletData.cryptocitizens_count;
        if (citizenCount >= 20)
            setStatus = 'ultra'; // Ultra Set (very rare)
        else if (citizenCount >= 10)
            setStatus = 'full'; // Full Set
        else if (citizenCount >= 3)
            setStatus = 'partial'; // Multiple cities
        return {
    ...
    async getEnhancedMarketInsights() {
        ...
        try {
            ...
            const duneData = await citizen_dune_integration_1.citizenDune.getBrightMomentsData();
            const duneInsights = await citizen_dune_integration_1.citizenDune.getMarketInsights();
            const holderAnalysis = await citizen_dune_integration_1.citizenDune.getHolderAnalysis();
            if (!duneData) {
                return {
                    dune_verified: false,
    ...
    async validateDataConsistency() {
        ...
        try {
            ...
            openSeaStats.forEach(openSeaCollection => {
                const duneCollection = duneData.collections.find(dc => dc.name.toLowerCase().includes(openSeaCollection.city.toLowerCase()));
                if (duneCollection) {
                    matchingCollections++;
...
exports.CitizenMarketData = CitizenMarketData;
exports.citizenMarketData = new CitizenMarketData();
