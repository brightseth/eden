...
exports.bartSDK = exports.BartClaudeSDK = void 0;
const sdk_1 = __importDefault(require("@anthropic-ai/sdk"));
const bart_gondi_integration_1 = require("./bart-gondi-integration");
class BartClaudeSDK {
    constructor(apiKey) {
     ...
     */
    async evaluateLoan(loanRequest) {
        try {
            // First, get real market data from Gondi
            const marketData = await bart_gondi_integration_1.bartGondiService.getMarketData();
            const collections = await bart_gondi_integration_1.bartGondiService.getCollectionData();
            ...
            if (loanRequest.collateralNFT.collection.startsWith('0x')) {
                nftEvaluation = await bart_gondi_integration_1.bartGondiService.evaluateNFT(loanRequest.collateralNFT.collection, loanRequest.collateralNFT.tokenId || '1');
            }
     ...
     */
    async assessRisk(collectionName, timeframe = '30 days') {
        try {
            // Get real market data
            const marketData = await bart_gondi_integration_1.bartGondiService.getMarketData();
            const collections = await bart_gondi_integration_1.bartGondiService.getCollectionData();
            // Find the specific collection
            const collection = collections.find(c => c.name.toLowerCase().includes(collectionName.toLowerCase()));
            const prompt = `
...
exports.BartClaudeSDK = BartClaudeSDK;
buildLendingSystemPrompt();
string;
{
...
buildLoanEvaluationPrompt(loanRequest, LoanRequest, marketData ?  : bart_gondi_integration_1.GondiMarketData, collections ?  : bart_gondi_integration_1.GondiCollection[], nftEvaluation ?  : any);
string;
{
    ...
    const realCollectionData = collections?.find(c => c.name.toLowerCase().includes(collateralNFT.collection.toLowerCase()) ||
...
parseLoanDecision(response, string);
LoanDecision;
{
    try {
...
validateLoanDecision(decision, any);
boolean;
{
...
exports.bartSDK = new BartClaudeSDK(process.env.ANTHROPIC_API_KEY);
