...
exports.miyomiEdenVideoGenerator = exports.MiyomiEdenVideoGenerator = void 0;
class MiyomiEdenVideoGenerator {
    constructor() {
     ...
     */
    async generateArtisticVideo(pick) {
     ...
     */
    async createProjectConfiguration(pick) {
        const titles = [
    ...
    selectPrimaryFrequency(pick) {
        if (pick.confidence > 0.85)
            return 'visceral';
        if (pick.edge > 0.25)
            return 'experimental';
        if (pick.risk_level === 'high')
            return 'cinematic';
        if (pick.sector === 'ai')
            return 'dreamlike';
        return 'intimate';
    }
    selectSecondaryFrequency(pick, primary) {
        const mappings = {
     ...
     */
    async generateNarrativeArchitecture(pick, project) {
    ...
    generateDevelopment(pick, project) {
    ...
    generateRevelation(pick, project) {
    ...
    generateResonance(pick, project) {
        const resonances = [
    ...
    generateVoiceDirection(project) {
        const directions = {
     ...
     */
    async generateKeyframeChoreography(pick, project, segmentCount) {
        const keyframes = [];
    ...
    async generateKeyframeDescription(pick, project, progress) {
    ...
    getCameraEmotion(progress, project) {
        const emotions = {
    ...
    getTransitionType(index, total, project) {
        if (index === 0)
            return 'dissolve'; // Gentle opening
        if (index === total - 1)
            return 'dissolve'; // Gentle closing
        // Use emotional frequency to determine transition style
        if (project.emotionalFrequency.primary === 'visceral')
            return 'hard_cut';
        if (project.emotionalFrequency.secondary === 'unsettling')
            return 'hard_cut';
        if (project.emotionalFrequency.primary === 'dreamlike')
            return 'dissolve';
        return Math.random() > 0.6 ? 'match_cut' : 'dissolve';
    }
    getDriftDirection(progress) {
        if (progress < 0.33)
            return 'lateral';
        if (progress < 0.66)
            return 'depth';
     ...
     */
    async generateAnimationAlchemy(keyframes, project) {
        const videoSegments = [];
    ...
    async generateSingleSegment(keyframe, project) {
        if (!this.edenApiKey) {
    ...
    craftSegmentPrompt(keyframe, project) {
...
Cinematic quality, professional lighting, ${project.emotionalFrequency.primary} atmosphere with ${project.emotionalFrequency.secondary} undertones. 8-second duration, smooth motion, artistic composition.`;
    ...
    generateMotionInstructions(motion) {
        const instructions = [];
        instructions.push(`${motion.zoom}% breathing zoom`);
        instructions.push(`${motion.drift} camera drift`);
        if (motion.morph)
            instructions.push('subtle reality morphing');
        if (motion.particles)
            instructions.push('particle dissolution effects');
        return instructions.join(', ');
    }
    generateStyleModifiers(project) {
        const styles = {
     ...
     */
    async generateSonicArchitecture(project, script) {
        if (!this.edenApiKey)
            return null;
        try {
     ...
     */
    async generateVoiceover(script, project) {
        if (!this.edenApiKey)
            return null;
        try {
            const response = await fetch(`${this.edenBaseUrl}/v2/tasks/create`, {
     ...
     */
    async generatePosterManifestation(project, pick) {
        if (!this.edenApiKey)
...
Style: Cinematic poster design, professional typography, compelling visual composition, 16:9 aspect ratio, high production value, artistic but readable.`;
     ...
     */
    async generateArtistStatement(project, pick) {
...
exports.MiyomiEdenVideoGenerator = MiyomiEdenVideoGenerator;
// Export singleton instance
exports.miyomiEdenVideoGenerator = new MiyomiEdenVideoGenerator();
