...
exports.abrahamSDK = exports.AbrahamClaudeSDK = void 0;
...
const real_time_knowledge_integrator_1 = require("./real-time-knowledge-integrator");
class AbrahamClaudeSDK {
    constructor(apiKey) {
        this.trainingData = null;
        this.trainingDataLoaded = false;
     ...
     */
    async reflectOnCovenant(recentWorks) {
        const prompt = `
...
Format as JSON:
{
  ...
  "recommendations": ["recommendation1", "recommendation2"]
     ...
     */
    async generateThematicSeries(theme, numberOfWorks) {
        const works = [];
...
Provide as JSON with same structure as daily creation.`;
     ...
     */
    async generateCovenantDocumentation(purpose) {
        const prompt = `
  ...
  "metadata": {
    ...
    async chat(message, context) {
        try {
            ...
            const loreSystemPrompt = await lore_manager_1.loreManager.generateEnhancedSystemPrompt('abraham');
...
Respond authentically as ABRAHAM, incorporating your deep lore while staying present to the conversation. Keep responses profound yet accessible (2-4 sentences typically).`;
    ...
    async loadTrainingData() {
        if (!this.trainingDataLoaded) {
            try {
                this.trainingData = await (0, training_data_loader_1.loadAbrahamTrainingData)();
     ...
     */
    async getChatWithTraining(message, context) {
        // Load training data if not already loaded
        await this.loadTrainingData();
        // Get enhanced context based on the message
        const enhancedContext = await (0, training_data_loader_1.getEnhancedResponseContext)('abraham', message);
        try {
            // Build enhanced system prompt with training data
            let systemPrompt = await lore_manager_1.loreManager.loadAgentLore('abraham');
            if (enhancedContext) {
                systemPrompt += `\n\nYour Current Status:\n`;
                ...
                if (enhancedContext.recent_artworks) {
                    ...
                    enhancedContext.recent_artworks.slice(0, 3).forEach((artwork) => {
                ...
                if (enhancedContext.relevant_artworks && enhancedContext.relevant_artworks.length > 0) {
                    ...
                    enhancedContext.relevant_artworks.slice(0, 2).forEach((artwork) => {
     ...
     */
    async getChatWithRealTimeKnowledge(message, context) {
        try {
            // Load training data
            await this.loadTrainingData();
            // Get real-time context
            const realTimeContext = await real_time_knowledge_integrator_1.realTimeKnowledge.getRealTimeContext('abraham', message);
            ...
            if (realTimeContext) {
                ...
                if (realTimeContext.recentTrends?.length > 0) {
                    systemPrompt += `- Cultural Trends: ${realTimeContext.recentTrends.slice(0, 2).map((t) => t.topic).join(', ')}\n`;
                }
                if (realTimeContext.relevantEvents?.length > 0) {
                    systemPrompt += `- Recent Events: ${realTimeContext.relevantEvents.slice(0, 2).map((e) => e.title).join(', ')}\n`;
                }
    ...
    async searchArtworks(query) {
        return await (0, training_data_loader_1.searchAbrahamArtworks)(query);
    }
    /**
     * Get specific artwork details
     */
    async getArtwork(artworkId) {
        return await (0, training_data_loader_1.getAbrahamArtwork)(artworkId);
    }
    /**
     * Get updated covenant progress from training data
     */
    async getEnhancedCovenantProgress() {
        const progress = await (0, training_data_loader_1.getAbrahamCovenantProgress)();
...
exports.AbrahamClaudeSDK = AbrahamClaudeSDK;
// Export singleton instance
exports.abrahamSDK = new AbrahamClaudeSDK(process.env.ANTHROPIC_API_KEY);
