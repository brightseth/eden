...
exports.berthaClaude = exports.BerthaClaudeSDK = void 0;
exports.syncWithEdenIntelligence = syncWithEdenIntelligence;
const sdk_1 = __importDefault(require("@anthropic-ai/sdk"));
const config_1 = require("./config");
const training_data_loader_1 = require("../training-data-loader");
class BerthaClaudeSDK {
    constructor(config = config_1.berthaConfig, apiKey) {
    ...
    async analyzeOpportunity(assetData) {
...
Use the evaluation criteria weights from my configuration.`;
    ...
    async generateStrategy(portfolio) {
...
4. Risk-adjusted strategy for next quarter`;
    ...
    async processTrainerInterview(responses) {
...
5. Success metrics`;
    ...
    mockStrategyResponse(portfolio) {
        return {
            allocation: [
...
Respond to questions about art collecting, market analysis, artist evaluation, or collection strategy. Your responses should demonstrate deep knowledge and sophisticated judgment (2-4 sentences typically).`;
    ...
    mockProcessInterview(responses) {
        return {
            taste: {
                ...this.config.taste,
    ...
    async loadTrainingData() {
        if (!this.trainingDataLoaded) {
            try {
                this.trainingData = await (0, training_data_loader_1.loadBerthaTrainingData)();
     ...
     */
    async analyzeOpportunityWithTraining(assetData) {
        // Load training data if not already loaded
        await this.loadTrainingData();
        // Get enhanced context for the analysis
        const enhancedContext = await (0, training_data_loader_1.getEnhancedResponseContext)('bertha', `analyze ${assetData.name} collection ${assetData.collection} price ${assetData.currentPrice}`);
...
Social Metrics: ${JSON.stringify(assetData.socialMetrics || 'Not available')}`;
        ...
        if (assetData.collectorType && enhancedContext?.collector_profiles) {
            const relevantProfile = enhancedContext.collector_profiles.find((p) => p.profile_id === assetData.collectorType ||
...
- Budget Range: ${relevantProfile.typical_budget_range}`;
        ...
        if (enhancedContext?.market_models) {
            ...
            enhancedContext.market_models.slice(0, 2).forEach((model) => {
...
5. Buy/Hold/Sell recommendation with rationale`;
     ...
     */
    async getCollectorProfile(profileType) {
        return await (0, training_data_loader_1.getBerthaCollectorProfile)(profileType);
    }
    /**
     * Get market prediction for specific model
     */
    async getMarketPrediction(modelName) {
        return await (0, training_data_loader_1.getBerthaMarketPrediction)(modelName);
    }
    /**
     * Get enhanced chat response using training data
     */
    async getChatWithTraining(message) {
        await this.loadTrainingData();
        const enhancedContext = await (0, training_data_loader_1.getEnhancedResponseContext)('bertha', message);
     ...
     */
    createEnhancedAnalysis(assetData, claudeResponse, enhancedContext) {
...
exports.BerthaClaudeSDK = BerthaClaudeSDK;
// Export singleton instance
exports.berthaClaude = new BerthaClaudeSDK();
// Integration with Eden's Collection Intelligence
async function syncWithEdenIntelligence(edenData, claudeAnalysis) {
