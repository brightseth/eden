...
const profileConfigs = __importStar(require("@/lib/profile/profile-config"));
class RegistryClient {
    ...
    transformRegistryAgent(registryAgent) {
        return {
            id: registryAgent.id,
    ...
    async getAgent(handle) {
        ...
        try {
            ...
            const registryAgent = await response.json();
    ...
    async getAgentWorks(handle, limit = 10) {
    ...
    async getAgentLore(handle) {
        const cacheKey = `lore:${handle}`;
        // Check cache first
        const cached = this.getCachedData(cacheKey);
        if (cached) {
            return { data: cached, source: 'cache' };
        }
        // Check Registry availability - try local dev first, then production
        const localUrl = 'http://localhost:3000/api/v1';
        const productionUrl = this.baseUrl;
        for (const baseUrl of [localUrl, productionUrl]) {
            try {
                const response = await fetch(`${baseUrl}/agents/${handle}/lore`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                if (!response.ok) {
                    console.warn(`[Registry] Failed to fetch lore for ${handle} from ${baseUrl}:`, response.status);
                    continue;
                }
                const data = await response.json();
                const lore = data.lore;
                this.setCachedData(cacheKey, lore);
                console.log(`✅ [Registry] Successfully fetched lore for ${handle} from ${baseUrl}`);
                return { data: lore, source: 'registry' };
            }
            catch (error) {
                console.warn(`[Registry] Error fetching lore for ${handle} from ${baseUrl}:`, error);
                continue;
            }
        }
        // No Registry source available - return fallback lore
        console.warn(`[Registry] No lore available for ${handle}, using fallback`);
        return this.getFallbackLore(handle);
    }
    async getAllAgents() {
        ...
        try {
            ...
            const registryAgents = await response.json();
    ...
    getFallbackConfig(handle) {
        ...
        const config = profileConfigs[configName];
        if (!config) {
            return {
    ...
    getFallbackLore(handle) {
        // Use the local lore files as fallback
        if (handle.toLowerCase() === 'abraham') {
            try {
                // Import Abraham lore dynamically if available
                const { abrahamLore } = require('@/data/agent-lore/abraham-lore');
                const lore = {
                    version: '1.0.0',
                ...
                return { data: lore, source: 'fallback' };
            }
            catch (error) {
                console.error('[Registry] Failed to load Abraham lore fallback:', error);
            }
        }
        return {
            error: `No lore available for ${handle}`,
            source: 'fallback'
        };
    }
    getFallbackAgents() {
    ...
    subscribeToAgent(handle, callback) {
    ...
    subscribeToWorks(handle, callback) {
