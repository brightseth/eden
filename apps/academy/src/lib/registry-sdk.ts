// Registry SDK v2 - Embedded for Academy deployment
...
export class RegistryClient {
  ...
  async getAgent(handle: string): Promise<Agent | null> {
    try {
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), this.config.timeout);
      ...
      return FALLBACK_AGENTS.find(agent => agent.handle === handle) || null;
    }
  }
  ...
  async searchAgents(query: string): Promise<Agent[]> {
    try {
      ...
      return FALLBACK_AGENTS.filter(agent => 
