...
export class MiyomiVideoGenerator {
   ...
   */
  private async generateScript(pick: MarketPick, duration: number, style: string): Promise<{
    text: string;
    segments: Array<{
      start: number;
      end: number;
  ...
  private generateHook(pick: MarketPick, style: string): string {
    const hooks = {
      spicy: [
  ...
  private generateAnalysis(pick: MarketPick, wordCount: number): string {
  ...
  private generateCTA(pick: MarketPick): string {
    const ctas = [
  ...
  private segmentScript(script: string, duration: number): Array<any> {
  ...
  private getVisualForSegment(time: number, totalDuration: number): string {
    const position = time / totalDuration;
    
    if (position < 0.2) return 'hook-animation';
    if (position < 0.4) return 'market-chart';
    if (position < 0.6) return 'data-visualization';
    if (position < 0.8) return 'position-display';
    return 'cta-screen';
  }
   ...
   */
  private async generateVisuals(pick: MarketPick, style: string, format: string): Promise<VideoAssets> {
  ...
  private async generateCharts(pick: MarketPick, aspectRatio: string): Promise<string[]> {
  ...
  private async generateSingleChart(type: string, pick: MarketPick, aspectRatio: string): Promise<string> {
    try {
  ...
  private async generateDataViz(pick: MarketPick, style: string, aspectRatio: string): Promise<string[]> {
    // Generate data visualizations
    return [
  ...
  private async generateLowerThirds(pick: MarketPick, style: string): Promise<string[]> {
  ...
  private async getBackgroundVideo(sector: string, style: string): Promise<string> {
  ...
  private async getMusicTrack(style: string): Promise<string> {
    const tracks = {
   ...
   */
  private async addBrandingEffects(video: any, style: string): Promise<any> {
    try {
   ...
   */
  async generateAndDistribute(pick: MarketPick): Promise<{
  ...
  private determineStyle(pick: MarketPick): 'spicy' | 'analytical' | 'contrarian' | 'celebratory' {
    if (pick.confidence > 0.85) return 'spicy';
    if (pick.edge > 0.25) return 'contrarian';
    if (pick.risk_level === 'low') return 'analytical';
    return 'contrarian'; // Default
  }
}

// Export singleton instance
export const videoGenerator = new MiyomiVideoGenerator();